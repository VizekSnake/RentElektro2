...
def create_reviewable_tool(db_session, owner_id: int) -> ToolModel:
    tool = ToolModel(
        Type="drill",
...
def test_add_review_and_read_summary(auth_client, db_session, test_user):
    ...
    assert create_response.json()["tool_id"] == tool.id
    assert create_response.json()["user_id"] == test_user.id
